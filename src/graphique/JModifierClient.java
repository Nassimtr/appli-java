...
public class JModifierClient extends JFrame {
	 ...
	 * Create the frame.
	 */
	public JModifierClient(Client client, LesClients lesClients, ListeTarifs listeTarifs) {
        ...
        txt_jourNaiss.setText(String.valueOf(client.getJourNaiss()));
        txt_moisNaiss.setText(String.valueOf(client.getMoisNaiss()));
        txt_anneeNaiss.setText(String.valueOf(client.getAnneeNaiss()));
        txt_villeNaiss.setText(client.getVilleNaiss());
        txt_paysNaiss.setText(client.getPaysNaiss());
        txt_nationalite.setText(client.getNationalite());
        txt_adresse.setText(client.getAdresse());
        txt_cp.setText(client.getCp());
        txt_ville.setText(client.getVille());
        txt_tel1.setText(client.getTel1());
        txt_tel2.setText(client.getTel2());
        txt_courriel.setText(client.getCourriel());
        txt_nomRespLegal.setText(client.getNomResp());
        txt_prenomRespLegal.setText(client.getPrenomResp());
		
        
        txt_nom.setEnabled(false);
        txt_prenom.setEnabled(false);
        txt_jourNaiss.setEnabled(false);
        txt_moisNaiss.setEnabled(false);
        txt_anneeNaiss.setEnabled(false);
        
        if (clientAModifier.getGenre().equals("Homme")) {
            rbtn_homme.setSelected(true);
        } else if (clientAModifier.getGenre().equals("Femme")) {
            rbtn_femme.setSelected(true);
        }

        switch (clientAModifier.getArme()) {
        case "Fleuret":
            rbtnFleuret.setSelected(true);
            break;
        case "Epee":
            rbtnEpee.setSelected(true);
            break;
        case "Sabre":
            rbtnSabre.setSelected(true);
            break;
        }

        if (clientAModifier.getLateralite().equals("Droitier")) {
            rbtnDroitier.setSelected(true);
        } else if (clientAModifier.getLateralite().equals("Gaucher")) {
            rbtnGaucher.setSelected(true);
        }

        if (clientAModifier.getPratique().equals("Loisir")) {
            rbtnLoisir.setSelected(true);
        } else if (clientAModifier.getPratique().equals("Competition")) {
            rbtnCompetition.setSelected(true);
        }

        chckbx_licenceSansAssurance.setSelected(clientAModifier.isLicenceSansAssurance());
        chckbx_licenceAssuranceRenforcee.setSelected(clientAModifier.isLicenceAssuranceRenforcee());
        chckbx_deuxiemeMembreFamille.setSelected(clientAModifier.isDeuxiemeMembreFamille());
        chckbx_troisiemeMembreFamille.setSelected(clientAModifier.isTroisiemeMembreFamille());
		    ...
		    public void actionPerformed(ActionEvent e) {
		        if (verifierChamps()) {
		            try {
		                ...
		                Client client = modifierClient();
		                if (client != null) {
		                    lesClients.ajouterClient(client);
		                    new SerialisationClient().serialiseClient(lesClients.getListeClients());
		                    JOptionPane.showMessageDialog(JModifierClient.this, "Adhérent modifier avec succès !");
		                }
		            } catch (Exception ex) {
		                ex.printStackTrace(); // Imprime l'exception dans la console
		            }
		        } else {
		            JOptionPane.showMessageDialog(JModifierClient.this, "Veuillez remplir tous les champs et sélectionner les options appropriées.", "Erreur", JOptionPane.ERROR_MESSAGE);
		        }
		    }
		});
		
		
		btnModifier.setBounds(554, 541, 85, 21);
	...
	public Client modifierClient() {
	    ...
	    Client clientModifie = new Client(nom, prenom, genre, jourNaiss, moisNaiss, anneeNaiss, villeNaiss, 
	    ...
	    lesClients.supprimerClient(clientAModifier.getNom(), clientAModifier.getPrenom(), clientAModifier.getJourNaiss(), clientAModifier.getMoisNaiss(), clientAModifier.getAnneeNaiss());
	    lesClients.ajouterClient(clientModifie); 
